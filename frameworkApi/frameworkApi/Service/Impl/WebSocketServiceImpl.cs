...
using System.Linq;
namespace frameworkApi.Service.Impl
{
   /// <summary>
    /// websocket实现类
    /// </summary>
    public class WebSocketServiceImpl : IWebSocketService
    {
        ...
        {
            sokcetServer = new WebSocketServer(string.Format("ws://{0}:{1}" ,ip,port));
        ...
        private void DoOpen(IWebSocketConnection socket)
        {
            ...
            socket.ConnectionInfo.Headers.TryGetValue("Authorization", out  token);
